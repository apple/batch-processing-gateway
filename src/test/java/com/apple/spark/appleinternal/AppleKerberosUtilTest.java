...
public class AppleKerberosUtilTest {
    ...
    public static final String CONTAINER_IMAGE = "docker.apple.com/aiml-datainfra/delegation-token-tool:snapshot-latest";
    ...
    public void secureHMSOnRequestAndAppConfigTest() {
        ...
        SubmitApplicationRequest request = ApplicationSubmissionHelper.parseSubmitRequest(REQUEST_BODY_JSON, CONTENT_TYPE);
        ...
        List<InitContainer> driverInitContainers = Collections.singletonList(new InitContainer(DELEGATION_CONTAINER_NAME, CONTAINER_IMAGE));
        ...
        Assert.assertEquals(sparkSpec.getDriver().getInitContainers().get(0).getEnv().get(0).getName(), INIT_CONTAINER_ENV_KEY);
        Assert.assertEquals(sparkSpec.getDriver().getInitContainers().get(0).getEnv().stream()
                .filter(envVar -> envVar.getName().equals(INIT_CONTAINER_ENV_KEY)).count(), 1);

        String[] envArray = sparkSpec.getDriver().getEnv().stream().map(EnvVar::getName).toArray(String[]::new);
        Assert.assertArrayEquals(envArray, new String[]{"HADOOP_USER_NAME", "HADOOP_TOKEN_FILE_LOCATION", "SPARK_USER"});

        Assert.assertEquals(Optional.ofNullable(sparkSpec.getDriver().getSecurityContext().getFsGroup()), Optional.ofNullable(1000L));
    }
    ...
    public void noSecureHMSOnRequest() {
        String REQUEST_BODY_NO_HMS_JSON =
                "{\n"
                        ...
                        + "}";
        SparkApplicationSpec sparkSpec = new SparkApplicationSpec();
        SubmitApplicationRequest request = ApplicationSubmissionHelper.parseSubmitRequest(REQUEST_BODY_NO_HMS_JSON, CONTENT_TYPE);
        TimerMetricContainer timerMetric = new TimerMetricContainer(meterRegistry);
        DriverSpec driverSpec = new DriverSpec();
        sparkSpec.setDriver(driverSpec);
        List<InitContainer> driverInitContainers = Collections.singletonList(new InitContainer(DELEGATION_CONTAINER_NAME, CONTAINER_IMAGE));
    ...
    public void secureHMSOnRequestAndNoInitContainerOnAppConfig() {
        ...
        DriverSpec driverSpec = new DriverSpec();
        sparkSpec.setDriver(driverSpec);
        Exception exception = Assert.assertThrows(WebApplicationException.class, () -> {
            AppleKerberosUtil.enableKerberosSupport(sparkSpec, request, appConfig, proxyUser, timerMetric);
        });
    ...
    public void secureHMSOnRequestAndNoDelegationInitContainerOnAppConfig() {
        ...
        DriverSpec driverSpec = new DriverSpec();
        sparkSpec.setDriver(driverSpec);
        List<InitContainer> driverInitContainers = Collections.singletonList(new InitContainer("no-delegation-token-tool", CONTAINER_IMAGE));
        appConfig.setDriverInitContainers(driverInitContainers);
        Exception exception = Assert.assertThrows(WebApplicationException.class, () -> {
            AppleKerberosUtil.enableKerberosSupport(sparkSpec, request, appConfig, proxyUser, timerMetric);
        });
        String expectedMessage = "Secured HMS can't be used without having delegation-token-tool initContainer in appConfig";
        String actualMessage = exception.getMessage();
        Assert.assertEquals(actualMessage, expectedMessage);
    }
}
