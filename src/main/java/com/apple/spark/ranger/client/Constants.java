package com.apple.spark.ranger.client;

public class Constants {

  // These values are used as certain suffixes for variables in RangerClient
  public static final String POLICY_REST_URL = "policy.rest.url";
  public static final String POLICY_REST_SSL_CONFIG_FILE = "policy.rest.ssl.config.file";
  public static final String POLICY_REST_CLIENT_CONNECTION_TIMEOUT =
      "policy.rest.client.connection.timeoutMs";
  public static final String POLICY_REST_CLIENT_READ_TIMEOUT = "policy.rest.client.read.timeoutMs";
  public static final String PLUGIN_USER = "plugin.user";
  public static final String PLUGIN_PASSWORD = "plugin.password";
}
